...
async function validaTypeNumber (req, res, next){
    if (typeof valor ==='number' && !Number.isNaN(valor)) {
        ...
        console.log(`Parámetro ingresado no es un número. Parámetro ingresado:${req.query.cant} `);
...
async function notZero (req, res, next){
    if (valor > 0){
        ...
        const msj = {
            ...
            console.log(`Número ingresado debe ser mayor a cero. Parámetro ingresado:${req.query.cant}`);
        res.status(400).json(msj);
    }
}
